...
def main():
    ...
    pygame.init()
    display = (min(np.size(matrix,1)*100,800),min(np.size(matrix,0)*100,800))
    ...
    #glTranslatef( -5, -5,np.size(matrix)*-1)
    glTranslatef(((np.size(matrix,1)*tileSize)/(-2)), ((np.size(matrix,0)*tileSize)/(-2)),-10)
